...
func generatePolicy(stringKey, principalId, effect, resource string) events.APIGatewayCustomAuthorizerResponse {
	...
	if effect != "" && resource != "" {
		...
		//CHECK IF I CAN PULL THIS FROM event.requestContext.authorizer.stringKey in GetUser
...
func handler(event events.APIGatewayCustomAuthorizerRequest) (events.APIGatewayCustomAuthorizerResponse, error) {
	...
	err := verifyJWT(token)
	//if error making new JWT key (this doesn't use a refresh token)
	if err != nil {
		return generatePolicy(err.Error(), "user", "Deny", event.MethodArn), nil
	}

	ret_email := claims.Email
	if ret_email == "" {
		return generatePolicy("no email providded in JWT", "user", "Deny", event.MethodArn), nil
	}

	//give keys back
	ret, err := json.Marshal(REGPackage{Token: token, Email: ret_email})
	if err != nil {
